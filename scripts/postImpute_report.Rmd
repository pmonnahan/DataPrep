...
chrom_file %>% filter(!V3 %in% c("frq")) %>% pivot_wider(id_cols=c("V1","V2"),names_from="V3",values_from="V4",values_fn=mean) %>% mutate(Dataset=V1,Chromosome=V2, HWE = hwe/raw, Missingness=lmiss/raw,MBS=missing/raw,MBC=
caseControlmissing/raw,Total=txt/raw,Duplicates=dupvar/raw,Indels = (txt - lmiss - dupvar - missing - caseControlmissing)/raw) %>% select(c(Dataset,Chromosome,HWE,Missingness,MBS,MBC,Duplicates,Indels)) %>% pivot_longer(c(HWE,Missingness,MBS,MBC,Duplicates,Indels),names_to = "Filter", values_to = "Proportion") %>% ggplot(aes(x=Dataset,fill=Filter,y=Proportion)) + geom_bar(stat="identity") + facet_wrap(~Chromosome) + theme(axis.text.x=element_text(angle=45,hjust=1))
```
